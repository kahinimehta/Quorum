...
Same as `python3 cli.py dashboard` or `./dashboard`. Opens the browser automatically (`--no-browser` to skip). UI has three tabs: **Step 1** static grant proposal (`graded6`, proposal-first layout with nested supporting-details group, not run-scoped); Steps 2–3 pipeline configure and results.
