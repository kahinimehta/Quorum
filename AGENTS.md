...
Same as `python3 cli.py dashboard` or `./dashboard`. Opens the browser automatically (`--no-browser` to skip). UI has three tabs: Steps 1–2 pipeline; **Step 3** static CUA grant proposal (`graded6`, proposal-first layout with grouped categories, not run-scoped).
