...
Same as `python3 cli.py dashboard` or `./dashboard`. Opens the browser automatically (`--no-browser` to skip). UI has three tabs: **Step 1** configure & run (full-width config panel + global run status strip); **Step 2** discovery results (ideal candidate profiles · ranked outputs · hypotheses, then pipeline diagram + collapsed **Audit & provenance**); **Step 3** static grant proposal (`graded6`, corpus vs verified-papers KPIs, proposal with pipeline diagram below aims + single **Pipeline trace & audit** collapse, not run-scoped).
