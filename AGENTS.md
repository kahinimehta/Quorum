...
Same as `python3 cli.py dashboard` or `./dashboard`. Opens the browser automatically (`--no-browser` to skip). UI has three tabs in a centered column (max 1400px): **Step 1** configure & run (+ global run status strip); **Step 2** discovery results (KPI strip · profiles · ranked outputs match hypotheses height, scroll when longer · hypotheses, then pipeline diagram + collapsed **Audit & provenance**); **Step 3** static grant proposal (`graded6`, corpus vs verified-papers KPIs, expanded proposal + collapsed **Grant pipeline** and **Pipeline trace & audit**, not run-scoped).
