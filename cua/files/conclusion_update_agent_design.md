...
```
 ...
 │      │ revise
 │      ▼
 │   F2 RIGOR BOOSTER [LLM]  ◀── un-selected corpus (the road not taken; live-only)
 │      │ hardens each aim's pitfalls + expected outcomes (F2 base, not the cap)
 │      ▼
...
**F2 Rigor Booster (as-built).** A binding-side, in-loop, *live-only* role on the revise path that
lifts the F2 *base* — rigor/feasibility **content** — as distinct from the evidence grades, which lift
the overclaim *cap* out-of-loop (fixed evidence, principle 6). Each revise round it mines the
**un-selected corpus** — the papers the Ingestion Layer (§7) bounded out of the writer's view, *"the road
not taken"* — for failure modes, alternative/contingency designs, and quantitative expected outcomes, and
rewrites each aim's `pitfalls_alternatives` + `expected_outcomes` with that substance (citing only corpus
ids; inv-1 intact). It feeds the **Reviser** (content), never the Critic: the Critic serializes only the
approach text and claims — never those two fields — so booster content cannot leak into the score
(principle 2). The **blinded eval judge** (§6), which reads the assembled proposal, is the arbiter, and its
F2 rises with the booster on. It is test-time compute that consults *more* of the corpus than the writer
ever saw. Offline it is inert, so the deterministic surrogate path is byte-identical.
...
stage — defined in the contract), and — as built — a binding-side **F2 Rigor Booster** on the revise
path (live-only; likewise an auxiliary, not a core stage). The deterministic two are the control and trust boundaries
...
### In-loop information integration — the F2 Rigor Booster (`cua/roles/_rigor_booster.py`)
v1.0 adds an **entrance for integrating information beyond the bounded writer view, mid-loop** — a reusable seam, of
which the **F2 Rigor Booster** is the first payload. The contribution is the *seam*, not F2: it shows new information
can enter the loop **without breaking the invariants**. On each revise round it reads the **un-selected corpus**
(`corpus.sources − writer_view` — the ~340 papers the Ingestion Layer held back, *"the road not taken"*), prioritized
by `possible_contradiction`, and rewrites each aim's `pitfalls_alternatives` + `expected_outcomes` from those
comparable studies the writer never saw. *Live-only* → the offline byte-oracle is unchanged.

*Why it's safe (the load-bearing part):* cites ∈ the **full** corpus (**inv-1**); it never re-grades evidence, so the
ladder stays fixed (**principle 6**); it feeds the **Reviser**, never the Critic — which serializes only approach text +
claims, so booster content **cannot leak into the score** (**principle 2**) — leaving the **blinded judge (§6)** the
independent arbiter. The same seam generalizes to any additional stream: the (still unfed) `external_critiques` channel,
freshly retrieved papers, steering inputs — the "continuous information" extension.

*Measured outcome — read with a grain of salt.* A matched, blinded A/B (`run_db --no-booster` for the baseline arm) over
8 runs: booster mean judge-F2 **5.25** (range 5–6) vs no-booster **4.5** (range 4–5). The judge does credit the booster's
pitfalls/outcomes by name, so the integration yields **real, judge-creditable rigor content**. But the number is **soft
on two counts** — the evidence grades are **uncalibrated** (`calibrated:False`), and the judge's F2 is a **coarse integer
(4/5/6) from a single model** — so the ~0.75 gap sits **within the measurement noise** and the arms overlap (baseline
drafts reach 5 unaided). The honest claim is *"a working in-loop information-integration seam that produces
judge-creditable rigor content,"* **not** a precise F2 lift. The single-baseline n=1 (judge 4→5) overstated it; the
matched arm is the honest comparator.

> **Grades update (closes the §7 "ungraded ceiling" residual above):** the upstream skeptic's evidence grades are now
> bound via a **local read-only snapshot** of the upstream evidence run through the agents-2 subgroup→connection→scoring
> logic (`build/integration/build_local_grades.py` → `local.sqlite`; **no shared-DB write**), reconciled by GRADE-BIND
> so claims resolve to their connection grade. On the real corpus grades read **`moderate`** → permitted L3 → the
> overclaim cap lifts (inv-3 clean). The grades remain a **heuristic surrogate** (`calibrated:False`); a validated
> skeptic is still the real fix. See `build/decisions/2026-06-03-{local-grades-snapshot,graded2-cap-vs-base,f2-booster-efficacy}.md`.

### Live demo — `outputs/graded6.html` (booster era) · `outputs/full2.*` (original ungraded-ceiling run)
