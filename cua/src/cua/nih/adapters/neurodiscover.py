...
from ...types import EvidenceEntry, Grade, GRADE_RANK, SourceSet
...
def task_from_db(conn, run_id, grant_call) -> NIHGrantTask:
    ...
    (#3); `EvidenceAssessment` keyed by CORPUS SOURCE ID (GRADE-BIND) with the backing connection's
    bucketed grade (min across connections; NULL→minimal) flagged uncalibrated; subgroups (#8) and
    treatments (#9). `commercial` omitted (#10)."""
    ...
    # EvidenceAssessment: keyed by CORPUS SOURCE ID — the id the writers actually cite (GRADE-BIND).
    # GATE-1 grading granularity is PRESERVED: the grade is STILL computed per connection from
    # `evidence_strength` (bucket_grade); only the resolution KEY changes (connection:{cid} → source id)
    # so `grounding.resolve_grade`'s direct dict lookup hits. A source backing >1 connection → the MIN
    # grade (§1.12 composite→lowest; conservative). A source in NO connection gets NO entry → a claim
    # citing it stays unscored→minimal (the honest coverage signal §2.5 — we do NOT fabricate a grade).
    source_grade: dict[str, Grade] = {}
    source_row: dict[str, dict] = {}
    treatments: list[Treatment] = []
    for c in conn_rows:
        cid = c["connection_id"]
        b = backing.get(cid, [])
        grade = bucket_grade(c.get("evidence_strength"))  # per-connection grade (GATE-1, unchanged)
        for row in b:
            sid = f'{row["source_type"]}:{row["source_id"]}'
            if sid not in source_grade or GRADE_RANK[grade] < GRADE_RANK[source_grade[sid]]:
                source_grade[sid] = grade  # min across the connections backing this source
            source_row.setdefault(sid, row)  # the source's own features (identical across connections)
        treatments.append(
            Treatment(
                name=c["treatment"],
                modality="",
    ...
    evidence = EvidenceAssessment()
    for sid in sorted(source_grade):  # deterministic key order
        evidence[sid] = _source_entry(sid, source_grade[sid], source_row[sid])
...
def _source_entry(source_id: str, grade: Grade, source_row: dict) -> EvidenceEntry:
    """One EvidenceAssessment entry PER CORPUS SOURCE (GRADE-BIND): keyed by the source id a claim
    cites, graded by the connection(s) that back it (the MIN grade across them; §1.12 composite→lowest),
    with features from the source's OWN row. The grade is still bucketed per connection from
    `evidence_strength` (GATE-1) — only the resolution key changed. Flagged UNCALIBRATED (GATE-1
    provenance) so no downstream reader mistakes a heuristic grade for a validated skeptic grade."""
    features = {
        "study_type": (source_row.get("study_type") or ""),
        "sample_size": source_row.get("sample_size"),
        "access_status": source_row.get("access_status"),
        **_UNCALIBRATED,  # REQUIRED — GATE-1 provenance flag
    }
    return EvidenceEntry(
        grade=grade,  # NULL→minimal; real ordinal Grade — bucketed per connection upstream of here
        evidence_features=features,
        support_source_ids=[source_id],  # the entry's own id (resolve_grade keys directly on it)
        caveats=[],
    )
