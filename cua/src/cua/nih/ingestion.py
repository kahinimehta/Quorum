...
# The attribute name the F2-booster reads off the corpus (see `roles/_rigor_booster`). It carries the set
# of corpus ids that are members of a mixed-polarity (possible_contradiction) cluster — the contrast
# signal the booster prioritizes when mining the UN-SELECTED corpus. Like `writer_view`, this is a
# binding-side instance attribute only; it touches neither `corpus.sources` nor the citation gate.
CONTRADICTION_IDS_ATTR = "possible_contradiction_ids"
...
def contradiction_source_ids(result: IngestionResult) -> set[str]:
    """The union of corpus ids in any `possible_contradiction` cluster (mixed negation polarity — the
    safety-net signal, guardrail 3). A plain id set — descriptive, NOT a grade. The F2-booster reads it to
    prioritize the contrasting / 'road-not-taken' un-selected papers a proposal should address."""
    return {mid for c in result.clusters if c.possible_contradiction for mid in c.member_ids}


def attach_contradiction_ids(corpus, result: IngestionResult) -> None:
    """Attach the `possible_contradiction` member-id set to the corpus for the F2-booster to read (mirrors
    `attach_writer_view`). Binding-side instance attribute only; `corpus.sources` and inv-1 are untouched.
    Offline-irrelevant: the fixture oracle never runs ingestion, so this attribute is never set there."""
    setattr(corpus, CONTRADICTION_IDS_ATTR, contradiction_source_ids(result))
