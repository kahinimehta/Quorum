...
class Orchestrator:
    ...
    def run(self, task: Task, run_config: RunConfig) -> tuple[Artifact, "object", "object"]:
        ...
        while True:
            ...
            # generated; round n = after the nth revise). Eagerly serialized — no aliasing.
            ...
            recorder.record(
                round, "orchestrator", output_ref=f"decision@r{round}", decision="pass" if passed else "revise",
            )
            if passed or round >= run_config.max_rounds - 1:
                break
