...
def test_grades_keyed_by_source_id_and_flagged_uncalibrated(seeded):
    # GRADE-BIND: the EvidenceAssessment is keyed by CORPUS SOURCE ID (source_type:source_id) — the id
    # the writers actually cite — NOT connection:{cid}. The keys are exactly the corpus sources that
    # back ≥1 connection; each is graded by its backing connection (here NULL→minimal) + flagged.
    seed, task = seeded["seed"], seeded["task"]
    ev = task.evidence
    corpus_ids = {p.id for p in task.source_set.sources}
    sid2type = {e["source_id"]: e["source_type"] for e in seed["evidence"]}
    backed = {f'{sid2type[s]}:{s}' for tc in seed["treatment_connections"] for s in tc["supporting_source_ids"]}
    assert set(ev.keys()) == backed                        # one entry per scored source (re-keyed)
    assert set(ev.keys()) <= corpus_ids                    # every key is a real corpus source id
...
# === GRADE-BIND positive proof: a claim citing a SCORED source resolves to its connection's grade ====
# Self-contained scored snapshot (non-NULL evidence_strength) — the seed above is all-NULL, so this is
# where the binding + the MIN-grade rule + the uncovered→unscored coverage signal are actually proven.

def _scored_db(path: str) -> None:
    """A tiny SCORED snapshot (reusing `_SCHEMA`): source 28598856 backs only a MODERATE connection (60);
    source 'Lw' backs that MODERATE one AND a WEAK one (30) → min ⇒ weak; grant 'G1' backs NO connection
    → no entry."""
    c = sqlite3.connect(path)
    try:
        c.executescript(_SCHEMA)
        for st, sid in [("literature", "28598856"), ("literature", "Lw"), ("grant", "G1")]:
            c.execute("INSERT INTO evidence(source_type, source_id, study_type) VALUES(?,?,?)", (st, sid, "cohort"))
        eid = {r[1]: r[0] for r in c.execute("SELECT evidence_id, source_id FROM evidence")}
        c.execute("INSERT INTO subgroups(name) VALUES('sg')")
        sg = c.execute("SELECT subgroup_id FROM subgroups").fetchone()[0]
        a = c.execute("INSERT INTO treatment_connections(subgroup_id, treatment, evidence_strength) "
                      "VALUES(?,?,?)", (sg, "Tmoderate", 60.0)).lastrowid   # bucket → MODERATE
        b = c.execute("INSERT INTO treatment_connections(subgroup_id, treatment, evidence_strength) "
                      "VALUES(?,?,?)", (sg, "Tweak", 30.0)).lastrowid       # bucket → WEAK
        for cid, sid in [(a, "28598856"), (a, "Lw"), (b, "Lw")]:           # G1 backs nothing
            c.execute("INSERT INTO connection_evidence(connection_id, evidence_id) VALUES(?,?)", (cid, eid[sid]))
        c.commit()
    finally:
        c.close()


def test_claim_citing_scored_source_resolves_to_connection_grade(tmp_path):
    db = str(tmp_path / "scored.db")
    _scored_db(db)
    ro = sqlite3.connect(f"file:{db}?mode=ro", uri=True)
    try:
        task = task_from_db(ro, run_id="grade-bind", grant_call=default_grant_call())
    finally:
        ro.close()
    ev = task.evidence

    # re-keyed by source id; the uncovered grant source has NO entry (not fabricated).
    assert set(ev.keys()) == {"literature:28598856", "literature:Lw"}
    assert "grant:G1" not in ev
    assert ev["literature:28598856"].grade is Grade.MODERATE
    assert ev["literature:Lw"].grade is Grade.WEAK            # min(moderate, weak) — §1.12 composite→lowest

    def _probe(*eids):
        return resolve_grade(Claim(id="p", text="t", evidence_ids=list(eids)), ev)

    # THE FIX: a claim citing a scored source now RESOLVES (not minimal/unscored).
    assert _probe("literature:28598856") == (Grade.MODERATE, False)   # the relay's exact probe
    assert _probe("literature:Lw") == (Grade.WEAK, False)             # min-grade rule reaches the claim
    assert _probe("literature:28598856", "literature:Lw") == (Grade.WEAK, False)  # composite → lowest
    # uncovered / unknown source → stays the HONEST unscored→minimal signal (§2.5).
    assert _probe("grant:G1") == (Grade.MINIMAL, True)
    assert _probe("literature:not-in-corpus") == (Grade.MINIMAL, True)
