...
def test_grades_flagged_uncalibrated_gate1(smoke):
    ...
    `calibrated=False`) at connection granularity, so no downstream reader mistakes it for a calibrated
    skeptic grade. The flag lives on the adapter's EvidenceAssessment (the run input)."""
    ev = smoke["task"].evidence
    assert ev and all(k.startswith("connection:") for k in ev)  # connection-granularity claim ids
