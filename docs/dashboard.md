...
| **Step 3** | **Grant Proposal (CUA)** — **static** bundled `graded6` NIH R01 demo (never changes with Step 1/2 `run_id`). Hero KPIs + green/amber/red audit status chips, **Jump to** section nav, full report **inlined** (numbered data-loop diagram, no nested frame). **Run a live CUA proposal (CLI)** block is expanded by default. Live runs: `python -m cua.nih.run_db` (see [Conclusion Update](workflow/conclusion-update#cua-package-optional--nih-grant-proposal)) |
