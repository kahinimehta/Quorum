...
| Run status stuck on Running | Hard-refresh after updating dashboard; global run status strip should show **Running** then **Complete** when `GET /api/run-discovery/status` finishes |
