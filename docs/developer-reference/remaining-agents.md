...
| Conclusion Update (CUA) | `cua/` — read-only; local proposal artifacts; Step 3 Grant Proposal tab |
...
## CUA — NIH grant proposal (Agent 6)
...
Outputs land under `cua/outputs/` (proposal, trace, audit, ingestion). No `recommendations` or `agent_outputs` writes. Full detail: [Conclusion Update workflow](../workflow/conclusion-update#cua-package--nih-grant-proposal) and `cua/README.md`.
