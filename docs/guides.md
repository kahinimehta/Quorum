...
Dashboard **Step 3** shows a **static** bundled demo (`graded6`) with hero KPIs (**corpus papers** = full pull, **verified papers** = top-K slice selected for drafting — see [Dashboard — Corpus vs verified papers](dashboard#corpus-vs-verified-papers)), **Jump to** nav (Proposal + supporting details), and an inlined report: **The proposal** (aims + **Grant pipeline** diagram) expanded at the top; pipeline/audit prose behind one collapsed **Pipeline trace & audit** group. Toolbar shows **`x/y obligations satisfied`**. To generate a new proposal from your DB:
