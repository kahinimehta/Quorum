...
- **Dashboard:** Step 3 **Grant Proposal** tab inlines the static bundled `graded6` report — **The proposal** expanded at top, **Grant pipeline** in its own collapsed block, then collapsed **Pipeline trace & audit**; **Jump to** → Proposal · Grant pipeline · Supporting details — not tied to Step 1/2 `run_id` (see [Dashboard — Step 3 report structure](../dashboard#step-3--report-structure))
