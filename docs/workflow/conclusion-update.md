...
- **Dashboard:** Step 3 **Grant Proposal (CUA)** tab inlines the static bundled `graded6` report — **Proposal first**, then four collapsed categories (Pipeline, Writer, Booster, Audit); jump-to nav opens Proposal or a category — not tied to Step 1/2 `run_id` (see [Dashboard](../dashboard#ui-layout))
