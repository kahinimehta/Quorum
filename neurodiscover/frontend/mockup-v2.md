...
Implemented highlights: **three tabs** (Configure & Run · Discovery Results · Grant Proposal), minimal header (DB LIVE + optional run chip), KPI strip, run status strip, full-width Step 1 configure panel, Step 2 results (profiles · ranked outputs match hypotheses height, scroll when longer · hypotheses, then pipeline diagram, audit & provenance collapsed), recent runs table, Step 3 static grant proposal (corpus/verified-papers KPIs, expanded **proposal**, collapsed **Grant pipeline** + **Pipeline trace & audit**, **`x/y obligations satisfied`**, Jump to nav).
