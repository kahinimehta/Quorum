...
4. **Step 3 — Grant Proposal** — static bundled `graded6` demo (not tied to your `run_id`). Hero KPIs (corpus · verified papers · citations · Critic F1/F2/F3), **Jump to** (Proposal + supporting details). **The proposal** stays expanded (aims + **Grant pipeline** diagram + Open full report); contract, booster, and audit prose sit behind **one** collapsed **Pipeline trace & audit** group. Report toolbar shows **`x/y obligations satisfied`**. **Run a live grant proposal (CLI)** is collapsible at the bottom. For a live run: `cd cua && pip install -e .` then `python -m cua.nih.run_db` with `ANTHROPIC_API_KEY` + `CUA_LIVE=1` (see `cua/README.md`).
