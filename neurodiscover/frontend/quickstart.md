...
4. **Step 3 — Grant Proposal (CUA)** — static bundled `graded6` demo (not tied to your `run_id`). Hero KPIs, audit status chips, **Jump to** (Proposal + Pipeline / Writer / Booster / Audit categories). The **NIH proposal appears first**; pipeline trace, writer contract, booster outcomes, and audit/ingestion are grouped in collapsed categories below. **Run a live CUA proposal (CLI)** is collapsible at the bottom. For a live run: `cd cua && pip install -e .` then `python -m cua.nih.run_db` with `ANTHROPIC_API_KEY` + `CUA_LIVE=1` (see `cua/README.md`).
