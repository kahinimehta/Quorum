...
3. **Step 2 — Discovery Results** — ideal candidate profiles and ranked treatments match the research hypotheses column height (scroll when longer); hypotheses use scores from **this run’s** recommendations; discovery pipeline diagram below; **Audit & provenance** collapsed by default — expand for agent trace + evidence table.
