...
3. **Step 2 — Discovery Results** — ideal candidate profiles, ranked treatments (panel matches profiles height, scrolls when the confidence list is longer), hypotheses (scores from **this run’s** recommendations); discovery pipeline diagram below; **Audit & provenance** collapsed by default — expand for agent trace + evidence table.
