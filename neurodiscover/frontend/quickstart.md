...
3. **Step 2 — Discovery Results** — ideal candidate profiles, ranked treatments (scrollable panel when the confidence list is long), hypotheses (scores from **this run’s** recommendations); discovery pipeline diagram below; **Audit & provenance** collapsed by default — expand for agent trace + evidence table.
